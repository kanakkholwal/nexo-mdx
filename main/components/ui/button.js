...
const buttonVariants = cva("inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-semibold tracking-wide ring-offset-background transition-transform transition-duration-300 transition transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50", {
    variants: {
        variant: {
            ...
            destructive: "bg-red-100 hover:bg-red-200 text-red-600	dark:bg-red-700 dark:text-red-200 dark:hover:bg-red-800 dark:hover:text-red-200",
            outline: "border border-input bg-accent hover:bg-accent/80 hover:text-accent-foreground dark:bg-gray-800 dark:border-gray-700 dark:text-white",
            secondary: "bg-secondary text-secondary-foreground hover:bg-secondary/90",
            ghost: "bg-slate-100 hover:bg-white shadow hover:shadow-lg hover:text-primary dark:bg-gray-800 dark:text-white dark:hover:bg-gray-700 dark:hover:text-white",
...
const Button = React.forwardRef(({ className, variant, size, asChild = false, rounded, transition, width, ...props }, ref) => {
    const Comp = asChild ? Slot : "button";
    return (_jsx(Comp, { className: cn(buttonVariants({ variant, size, className, transition, rounded, width })), ref: ref, type: "button", ...props }));
});
